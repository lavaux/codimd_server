...
class OAuth2CustomStrategy extends Strategy {
  ...
  userProfile (accessToken, done) {
    ...
    self._oauth2.get(self._userProfileURL, accessToken, function (err, body, res) {
      ...
      self._oauth2.get(self._workspaceURL, accessToken, function (err_w, body_w, res_w) {
        let json_w

        if (err_w) {
          logger.error(print(err_w))
          return done(new InternalOAuthError('Failed to fetch workspace profile', err_w))
        }
  
        try {
          json_w = JSON.parse(body_w)
        } catch (ex) {
          return done(new Error('Failed to parse workspace profile'))
        }

        const profile = parseProfile(json, json_w)
...
function parseProfile (data, data_workspace) {
  ...
  const workspaces = data_workspace["values"]

  logger.info('workspaces are ' + workspaces)
  return {
    id: id || username,
    username: username,
